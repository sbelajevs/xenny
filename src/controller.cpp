...
void Layout::init(GameState* gs)
{
    ...
    {
        int id = gameState->tableaux[i].handle;
        stackRects[id] = getCardScreenRect(
            tableausTopLeftX + (tableauInterval+CARD_WIDTH)*i, 
            tableausTopLeftY
        );
    }
    for (int i=0; i<FOUNDATION_COUNT; i++)
    {
        int id = gameState->foundations[i].handle;
        stackRects[id] = getCardScreenRect(
            foundationsTopLeftX + (tableauInterval/2.f+CARD_WIDTH)*i, 
            foundationsTopLeftY
        );
    }
    stackRects[gameState->stock.handle] = getCardScreenRect(stockTopLeftX, stockTopLeftY);
    stackRects[gameState->waste.handle] = getCardScreenRect(wasteTopLeftX, wasteTopLeftY);
    stackRects[gameState->hand.handle] = getCardScreenRect(0.f, 0.f);
}
...
{
    if (stack->handle >= 0 && stack->handle < STACK_COUNT) {
        return stackRects[stack->handle];
    }
    return getCardScreenRect(-1.f, -1.f);
}

Rect Layout::getCardRect(const CardStack* stack, int idx) const
{
    Rect resultRect = getStackRect(stack);

    if ((stack->type == CardStack::TYPE_TABLEAU || stack->type == CardStack::TYPE_HAND) 
        && idx <= stack->size) 
    {
        for (int i=0; i<idx; i++) {
            resultRect.y += stack->data[i].state == GameCard::STATE_OPEN 
                ? CARD_OPEN_SLIDE 
                : CARD_CLOSED_SLIDE;
        }
    }
...
Rect Layout::getDestCardRect(const CardStack* stack) const
{
    return getCardRect(stack, stack->size);
}

CardStack* Layout::probePos(float x, float y, int* idx) const
{
    Rect rect;

    rect = getStackRect(&gameState->stock);
    if (rect.inside(x, y))
    {
        *idx = gameState->stock.size - 1;
        return &gameState->stock;
    }

    rect = getStackRect(&gameState->waste);
    if (rect.inside(x, y))
    {
        *idx = gameState->waste.size - 1;
        return &gameState->waste;
    }

    for (int i=0; i<TABLEAU_COUNT; i++)
    {
        CardStack* s = &gameState->tableaux[i];
        for (int j=s->size-1; j>=0 && s->data[j].state == GameCard::STATE_OPEN; j--)
        {
            Rect curCardRect = getCardRect(&gameState->tableaux[i], j);
            if (curCardRect.inside(x, y))
            {
                *idx = j;
                return &gameState->tableaux[i];
            }
        }
    }
    ...
    for (int i=0; i<FOUNDATION_COUNT; i++)
    {
        Rect rect = getStackRect(&gameState->foundations[i]);
        if (rect.inside(x, y))
        {
            *idx = gameState->foundations[i].size - 1;
            return &gameState->foundations[i];
        }
    }
...
void Layout::initHand(CardStack* stack, int idx, float x, float y)
{
...
void Layout::updateHandRect(float x, float y)
{
    stackRects[gameState->hand.handle] = getCardScreenRect(x + handDx, y + handDy);
}
...
void GameGUI::handleControls(const Input* in)
{
    ...
    int stackIdx = -1;
    CardStack* stack = layout->probePos(x, y, &stackIdx);
    ...
    else if (in->dragStart)
    {
        if (stack != NULL_PTR && stack->empty() == false)
        {
            ...
            {
                layout->initHand(stack, stackIdx, in->left.pressX, in->left.pressY);
    ...
    else if (in->dragActive)
    {
        if (gameState->hand.empty() == false) {
            layout->updateHandRect(in->x, in->y);
        }
    }
    else if (in->dragEnd) 
    {
...
void GameGUI::handleHandRelease()
{
    ...
    Rect handRect = layout->getCardRect(&gameState->hand, 0);
    float bestDist = -1.f;
